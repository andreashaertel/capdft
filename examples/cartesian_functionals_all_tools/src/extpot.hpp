...
void electrostatic_wavelike(
  ...
  for (size_t i = 0; i < grid_counts.at(0); ++i) {
    ...
    for (size_t j = 0; j < grid_counts.at(1); ++j) {
      ...
      wave_left = .5 + 2. * pow(sin(frequency * x), 2);
      wave_right =.5 + 2. * pow(sin(frequency * y), 2);
      k_left = static_cast<size_t>(wave_left * 0.3 / bin_sizes.at(2) + .5);
      k_right = 54;
      pos_left = {i, j, k_left};
  ...
  //size_t index{0};
  //double slope{0.};
  //slope = -2. * potential / static_cast<double>(grid_counts.at(2) - 1 - 2 * 10);
  //for (size_t i = 0; i < grid_counts.at(0); ++i) {
  //  for (size_t j = 0; j < grid_counts.at(1); ++j) {
  //    for (size_t k = 0; k < grid_counts.at(2); ++k) {
  //      index = i * grid_counts.at(2) * grid_counts.at(1) +
  //          j * grid_counts.at(2) + k;
  //      solution.at(index) = potential + slope * (static_cast<double>(k) - 10.);
  //    }
  //  }
  //}
  for (size_t i = 0; i < 64; ++i) { std::cerr << solution.at(i) << std::endl; }
  std::cerr << std::endl;
